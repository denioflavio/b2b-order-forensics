...
create or replace package body bf_cart_pkg as
 procedure require_session is
 begin
  if nvl(v('APP_ID'),'0')<>'107' or v('APP_SESSION') is null or v('APP_USER') is null
 ...
 begin
  ...
  select max(order_id) into l_order from bf_order where request_hash=l_key and attempt_no=2;
  if l_order is null then bf_order_pkg.receive_attempt(l_key,v('APP_SESSION'),v('APP_USER'),l_order); end if;
